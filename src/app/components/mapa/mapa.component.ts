...
// Leaflet icon fix para bundlers que no copian las imágenes automáticamente
...
export class MapaComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnChanges() {
    if (this.inicializado) this.actualizarMarcadores();
  }
  ...
  private inicializarMapa() {
    ...
    this.map = L.map(contenedor, { zoomControl: true }).setView([40.416775, -3.703790], 6);
  ...
  private actualizarMarcadores() {
    ...
    for (const g of this.gasolineras) {
      ...
      const estado = g.abierta ? '<span style="color:#86efac">● Abierta</span>' : '<span style="color:#fca5a5">● Cerrada</span>';
