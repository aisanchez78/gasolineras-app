...
export class GasolineraService {
  ...
  // Fase 1 — síncrono: calcula Haversine, filtra por radio/carburante/marca. Sin límite.
  filtrarCandidatos(gasolineras: Gasolinera[], posicion: Coordenadas, filtros: FiltrosActivos): Gasolinera[] {
    ...
    return gasolineras
      .map(g => {
        ...
        return { ...g, distancia: this.geo.calcularDistanciaKm(posicion.lat, posicion.lng, lat, lng), abierta: this.estaAbierta(g.Horario) };
      })
      .filter(g => {
  ...
  enriquecerConDistanciasReales(candidatos: Gasolinera[], posicion: Coordenadas): Observable<Gasolinera[]> {
    ...
    return this.osrm.calcularDistancias(posicion, destinos).pipe(
      switchMap(metros => {
        if (!metros) return of(preseleccion); // fallback Haversine
        return of(preseleccion.map((g, i) => ({ ...g, distancia: metros[i] / 1000 })));
      })
    );
  }
